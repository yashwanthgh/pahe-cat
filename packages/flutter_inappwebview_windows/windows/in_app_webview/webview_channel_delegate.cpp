...
namespace flutter_inappwebview_plugin
{
  ...
  // PAHE CAT PATCH ----------------------------------------------------------
  // The keys below are the field names of DownloadStartRequest in
  // flutter_inappwebview_platform_interface, which builds itself from a flat
  // map of exactly these. `url` and `contentLength` are non-nullable there, so
  // they are always sent; the rest go as null when WebView2 does not say.
  //
  // textEncodingName and userAgent have no WebView2 equivalent on a download,
  // so they are always null rather than invented.
  void WebViewChannelDelegate::onDownloadStartRequest(const std::string& url,
    ...
    const std::optional<std::string>& contentDisposition) const
  {
    ...
    auto arguments = std::make_unique<flutter::EncodableValue>(flutter::EncodableMap{
      ...
      {"textEncodingName", make_fl_value()},
      {"userAgent", make_fl_value()},
      });
