...
namespace flutter_inappwebview_plugin
{
  ...
  void InAppWebView::registerEventHandlers()
  {
    ...
    // PAHE CAT PATCH --------------------------------------------------------
    // Upstream never subscribes to DownloadStarting on Windows. The Dart side
    // of this plugin already handles an "onDownloadStartRequest" message, and
    // in_app_webview_settings.cpp already parses `useOnDownloadStart`, but
    // nothing here ever raised the event — so on Windows a download did not
    // merely get discarded, it never began.
    //
    // WebView2 has a real download engine, as WKWebView does on macOS, so this
    // is the same fix as the macOS one: let the transfer run, point it at a
    // folder Dart is watching, and report that it started. Dart judges
    // completion from the file on disk.
    //
    // Why the bytes have to be taken here: the media host's Cloudflare rules
    // refuse every request the app can make for itself — four header variants
    // through Dart's HTTP client and three fetch orderings inside a WebView
    // were all answered 403 — while allowing this navigation.
    ...
    if (SUCCEEDED(webView->QueryInterface(IID_PPV_ARGS(&webView4)))) {
      failedLog(webView4->add_DownloadStarting(
        Callback<ICoreWebView2DownloadStartingEventHandler>(
          ...
          {
            // Leaves stock behaviour alone for anyone not asking for this.
            if (!settings || !settings->useOnDownloadStart) {
              return S_OK;
            }

            wil::com_ptr<ICoreWebView2DownloadOperation> operation;
            if (failedAndLog(args->get_DownloadOperation(&operation)) || !operation) {
              return S_OK;
            }

            wil::unique_cotaskmem_string uri;
            const std::string url = SUCCEEDED(operation->get_Uri(&uri)) && uri
              ? wide_to_utf8(uri.get()) : "";

            wil::unique_cotaskmem_string mimeTypeRaw;
            const std::optional<std::string> mimeType =
              SUCCEEDED(operation->get_MimeType(&mimeTypeRaw)) && mimeTypeRaw
              ? std::optional<std::string>{ wide_to_utf8(mimeTypeRaw.get()) }
              : std::nullopt;

            wil::unique_cotaskmem_string dispositionRaw;
            const std::optional<std::string> contentDisposition =
              SUCCEEDED(operation->get_ContentDisposition(&dispositionRaw)) && dispositionRaw
              ? std::optional<std::string>{ wide_to_utf8(dispositionRaw.get()) }
              : std::nullopt;

            // -1 when the server sent no length. Passed on as it is; the Dart
            // side treats a non-positive length as "watch until it stops
            // growing" rather than assuming a total it was never told.
            INT64 contentLength = 0;
            if (failedAndLog(operation->get_TotalBytesToReceive(&contentLength))) {
              contentLength = -1;
            }

            // WebView2's own choice of path already ends in the filename it
            // derived from the response, which is what suggestedFilename means
            // elsewhere in this plugin.
            wil::unique_cotaskmem_string defaultPath;
            if (failedAndLog(args->get_ResultFilePath(&defaultPath)) || !defaultPath) {
              return S_OK;
            }
            const std::filesystem::path suggested(defaultPath.get());
            const std::filesystem::path filename = suggested.filename();
            if (filename.empty()) {
              return S_OK;
            }

            // The folder Dart watches. Dart's Directory.systemTemp resolves to
            // the same place GetTempPathW reports, which is what lets the two
            // sides agree on a path without passing one between them.
            WCHAR tempRoot[MAX_PATH];
            const DWORD tempLen = GetTempPathW(MAX_PATH, tempRoot);
            if (tempLen == 0 || tempLen > MAX_PATH) {
              debugLog("pahe cat: no temp path");
              return S_OK;
            }

            std::error_code ec;
            const std::filesystem::path dir =
              std::filesystem::path(std::wstring(tempRoot, tempLen)) / L"pahe_cat_downloads";
            std::filesystem::create_directories(dir, ec);
            if (ec) {
              debugLog("pahe cat: could not create " + wide_to_utf8(dir.wstring()));
              return S_OK;
            }

            const std::filesystem::path destination = dir / filename;
            // Without this WebView2 would keep the old file and download to
            // "name (1)", leaving Dart watching a path that never grows.
            std::filesystem::remove(destination, ec);

            if (failedAndLog(args->put_ResultFilePath(destination.c_str()))) {
              return S_OK;
            }
            // Suppresses WebView2's own download flyout, which would otherwise
            // appear over the app for a transfer the user did not ask to see.
            failedLog(args->put_Handled(TRUE));
      ...
      debugLog("pahe cat: ICoreWebView2_4 unavailable, downloads will not run");
    }
    // END PAHE CAT PATCH ----------------------------------------------------
