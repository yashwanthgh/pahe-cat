...
//            }
    ...
    public func download(_ download: WKDownload, decideDestinationUsing response: URLResponse, suggestedFilename: String, completionHandler: @escaping (URL?) -> Void) {
        ...
        // PAHE CAT PATCH -----------------------------------------------------
        // Upstream discards every download here:
        //
        //     download.delegate = nil
        //     completionHandler(nil)   // cancel the download
        //
        // It reports the URL to Dart and throws the bytes away. That URL is of
        // no use on its own: the media host's Cloudflare rules refuse every
        // request the app can make — four header variants through Dart's HTTP
        // client and three fetch orderings inside a WebView were all answered
        // 403 — while accepting this very navigation, which delivered
        // 237,645,137 bytes in 223ms. The bytes are only obtainable here.
        //
        // So the download is kept and written to a folder Dart can watch,
        // named by the host's own suggested filename. Only done when the app
        // has asked for download reporting, so behaviour is unchanged
        // otherwise.
        if let useOnDownloadStart = settings?.useOnDownloadStart, useOnDownloadStart {
            let dir = URL(fileURLWithPath: NSTemporaryDirectory())
                .appendingPathComponent("pahe_cat_downloads", isDirectory: true)
            do {
                try FileManager.default.createDirectory(
                    at: dir, withIntermediateDirectories: true)
                let destination = dir.appendingPathComponent(suggestedFilename)
                // A stale part-file would make the new one append to it.
                ...
                print("PaheCat: cannot prepare a download destination: \(error)")
            }
        }
    ...
    public func webView(_ webView: WKWebView, navigationResponse: WKNavigationResponse, didBecome download: WKDownload) {
        ...
        if let url = response.url, let useOnDownloadStart = settings?.useOnDownloadStart, useOnDownloadStart {
            let downloadStartRequest = DownloadStartRequest(url: url.absoluteString,
                                                            ...
                                                            textEncodingName: response.textEncodingName)
            channelDelegate?.onDownloadStartRequest(request: downloadStartRequest)
            // PAHE CAT PATCH: kept, where upstream sets this to nil. Clearing
            // it drops the download before decideDestinationUsing is ever
            // asked where to put the file.
